...
def parse_docx_with_chapters(content: bytes) -> List[Dict]:
    ...
    for para in doc.paragraphs:
        ...
        for pattern in MODULE_PATTERNS:
            ...
            if match:
                current_module = match.group(1).replace(' ', '') + "：" + match.group(2)
                break
        ...
        for pattern in CHAPTER_PATTERNS:
            ...
            if match:
                ...
                chapter_num = chinese_to_num(num_str) if not num_str.isdigit() else int(num_str)
        ...
        elif not is_chapter and not current_chapter:
            # 还没有遇到第一个章节，创建一个"前言"章节
            current_chapter = {
                "chapter_name": "前言",
                "chapter_num": 0,
                "module_name": current_module,
