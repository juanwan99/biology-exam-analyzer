...
from score_allocator import ScoreAllocator
...
class DifficultyEngine:
    ...
    def __init__(
        ...
        Args:
            rules_path: 规则库JSON文件路径（默认使用config中的路径）
            gemini_analyzer: Gemini分析器实例（用于LLM精调）
        """
        self.rules_path = rules_path or str(RULES_DIR / "difficulty_rules.json")
        self.rules = self._load_rules()
        self.gemini_analyzer = gemini_analyzer
        self.score_allocator = ScoreAllocator()
    ...
    def refine_with_llm(
        ...
        Args:
            ...
            analysis_result: Stage1的分析结果（包含答案、子题等）
        ...
        try:
            # 加载精调Prompt
            prompt_path = str(PROMPT_DIR / "difficulty_refine_prompt.txt")
            try:
            ...
            # 填充Prompt
            analysis_json = json.dumps(analysis_result, ensure_ascii=False, indent=2) if analysis_result else "{}"
            ...
            logger.info(f"[LLM精调] API响应长度: {len(response_text)}字符")
            logger.debug(f"[LLM精调] API完整响应: {response_text}")
            ...
            logger.info(f"[LLM精调] 新增字段: difficulty_factors_weighted={len(refined_result.get('difficulty_factors_weighted', []))}, cognitive_breakdown={'是' if refined_result.get('cognitive_breakdown') else '否'}, question_tags={len(refined_result.get('question_tags', []))}")
    ...
    def evaluate_with_refinement(
        ...
        """
        完整的难度评估流程：规则引擎 + LLM精调 + 分值分配

        Args:
            question: 题目信息（包含total_score, num_options等字段）
            mode: 评估模式
                - "fast" (快速模式): 仅规则引擎，耗时短，适合预览
                - "deep" (深度模式): 规则引擎 + LLM精调，耗时长，准确度高
            analysis_result: Stage1的分析结果（用于LLM精调时的答案验证和子题评分）

        Returns:
            完整评估结果（包含score_distribution_by_difficulty字段）
        """
        # 第1步：规则引擎基础评估
        ...
        if analysis_result:
            try:
                score_distribution = self.score_allocator.allocate_question_scores(
                    question,
                    analysis_result
                )
                logger.info(f"[分值分配] 题目 {question.get('id')} 分值分配: {score_distribution}")
            except Exception as e:
                logger.error(f"[分值分配] 题目 {question.get('id')} 分值分配失败: {str(e)}")
                # 使用默认分配：整体难度决定
                total_score = question.get("total_score", 0)
                overall_difficulty = base_result.get("base_difficulty", 5.0)
                difficulty_label = self.score_allocator.map_difficulty_score_to_label(overall_difficulty)
                score_distribution[difficulty_label] = total_score

        # 第3步：LLM精调（根据模式决定）
        if mode == "deep" and self.gemini_analyzer:
            logger.info(f"[评估] 题目 {question.get('id')} 使用深度模式（规则+LLM精调）")
            refined_result = self.refine_with_llm(question, base_result, analysis_result)
            ...
            if refined_result:
                ...
                return {
                    **base_result,
                    ...
                    "difficulty_factors_weighted": refined_result.get("difficulty_factors_weighted", []),
                    "cognitive_breakdown": refined_result.get("cognitive_breakdown", {}),
                    "question_tags": refined_result.get("question_tags", []),
                    "adjustment_reasons": refined_result.get("adjustment_reasons", ""),
                    "answer_verification": refined_result.get("answer_verification", {}),
                    "sub_difficulties": refined_result.get("sub_difficulties", []),
                    "score_distribution_by_difficulty": score_distribution
                }
            else:
        ...
        base_result["score_distribution_by_difficulty"] = score_distribution
        return base_result
